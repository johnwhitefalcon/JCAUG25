...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // 2. Request Image Generation from Gemini Imagen
    async function generateMatrixImage(matrix, labels) {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const prompt = `A clean, professional data visualization heatmap of a correlation matrix labeled with nodes [${labelsString}]. Visualized values: ${matrixString}. Show interconnected node networks with heat values expressed as color intensity on a dark clean theme.`;
    ...
    generateMatrixImage(calculatedMatrix, calculatedLabels);
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ marginBottom: '20px', minHeight: '320px' }}>
        <h3>M{iteration} Graph Visualization</h3>
        {loading && <p>Generating visual representation via Gemini Imagen...</p>}
        {error && <p style={{ color: 'red' }}>{error}</p>}
        {generatedImage && (
          <img
            src={generatedImage}
            alt={`M${iteration} Matrix Visualization`}
            style={{
              maxWidth: '400px',
              maxHeight: '400px',
              borderRadius: '8px',
