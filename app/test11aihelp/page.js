...
export default function Home() {
  ...
  // 3. Average Reciprocals & Track Combined Labels
  var recipextra = [];
  var newLabelName = '';
  ...
  function avgrecip() {
    for (var i = 0; i < unique2.length; i += 2) {
      ...
      // Form combined label name e.g., "5/19"
  ...
  // Update currentLabels list with new combined header
  if (newLabelName) {
  ...
  function m1graph() {
    const nodeCoords = {
      12: { x: 340, y: 50 }, 8: { x: 340, y: 120 }, 1: { x: 280, y: 170 },
      10: { x: 420, y: 200 }, 16: { x: 280, y: 230 }, 7: { x: 340, y: 230 },
      9: { x: 190, y: 290 }, 2: { x: 260, y: 290 }, 4: { x: 340, y: 290 },
      3: { x: 410, y: 290 }, 5: { x: 80, y: 230 }, 19: { x: 140, y: 230 },
      14: { x: 110, y: 310 }, 6: { x: 490, y: 230 }, 18: { x: 550, y: 230 }
    };

    const links = [
      { u: 12, v: 8, double: false }, { u: 8, v: 1, double: false },
      { u: 8, v: 10, double: false }, { u: 8, v: 7, double: true },
      { u: 1, v: 16, double: false }, { u: 16, v: 7, double: false },
      { u: 7, v: 10, double: false }, { u: 7, v: 4, double: false },
      { u: 9, v: 2, double: false }, { u: 2, v: 4, double: false },
      { u: 4, v: 3, double: true }, { u: 5, v: 19, double: true },
      { u: 19, v: 14, double: false }, { u: 6, v: 18, double: true }
    ];

    graphComponent = (
      <div style={{ padding: '15px', background: '#fff', marginBottom: '20px', border: '1px solid #ccc' }}>
        <h3>M{iteration} Graph</h3>
        <svg width="620" height="350" style={{ border: '1px solid #eee' }}>
          {links.map((link, idx) => {
            ...
            if (link.double) {
              ...
              return (
                <g key={idx}>
                  <line x1={p1.x + nx} y1={p1.y + ny} x2={p2.x + nx} y2={p2.y + ny} stroke="#333" strokeWidth="2" />
                  <line x1={p1.x - nx} y1={p1.y - ny} x2={p2.x - nx} y2={p2.y - ny} stroke="#333" strokeWidth="2" />
                </g>
              );
            }
            return <line key={idx} x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y} stroke="#333" strokeWidth="2" />;
          })}
          {Object.entries(nodeCoords).map(([id, pos]) => (
            <g key={id} transform={`translate(${pos.x}, ${pos.y})`}>
              <circle r="16" fill="#fff" stroke="#333" strokeWidth="2" />
              <text textAnchor="middle" dy="5" fontSize="13" fontWeight="bold" fill="#333">{id}</text>
            </g>
          ))}
        </svg>
      </div>
  ...
  // Store m1matrix & filter sentinel values
  let m1matrix = [];
  let m1Labels = [];
  ...
  function filterMatrix() {
    ...
    // Filter labels
    m1Labels = currentLabels.filter((_, idx) => !removeIndices.has(idx));
    ...
    // Ensure 0 on main diagonal
    for (let i = 0; i < m1matrix.length; i++) {
      if (m1matrix[i]) m1matrix[i][i] = 0;
    }
  }
  filterMatrix();

  // Handler to iterate steps using m1matrix as the new source matrix
  const handleIterate = () => {
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      {/* Controls to trigger repeating the process using m1matrix */}
